...
RUN existing_group="$(getent group "${OPENCODE_GID}" | cut -d: -f1 || true)" \
    ...
    && if [ -n "${existing_user}" ] && [ "${existing_user}" != opencode ]; then usermod --login opencode --home /home/opencode --shell /bin/bash "${existing_user}"; elif [ -z "${existing_user}" ]; then useradd --uid "${OPENCODE_UID}" --gid "${OPENCODE_GID}" --create-home --shell /bin/bash opencode; fi \
